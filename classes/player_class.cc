...
void Player::SetPosition() {
  if (direction == "UP") { x_body[0] -= 1; }
  if (direction == "RIGHT") { y_body[0] += 1; }
  if (direction == "DOWN") { x_body[0] += 1;  }
  if (direction == "LEFT") { y_body[0] -= 1; }
  // Body parts take the position of the body part in front.
...
// Return the x player body positions.
std::vector<int> Player::GetXBody() {
  return x_body;
}

// Return the y player body positions.
...
// Add a new body part to the snake player.
void Player::AddBody() {
